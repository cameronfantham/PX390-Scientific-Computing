  ...
  if(E==0) {
    ...
    for(j=0;j<ntheta;j++){
      for(p=0;p<nzeta;p++){
        ...
        if(j!=j0||p!=p0){
          //j-1,p
          if(is_valid(j-1,p,ntheta,nzeta)&&is_valid(j+1,p,ntheta,nzeta)) {
	    C = (Q11[indx(j+1,p,ntheta,nzeta)]-Q11[indx(j-1,p,ntheta,nzeta)])/(dtheta*dtheta);
            setv(&bmat,indx(j,p,ntheta,nzeta),indx(j-1,p,ntheta,nzeta),A-C);};
          /* Left Boundary Periodicity */
          if(!is_valid(j-1,p,ntheta,nzeta)&&is_valid(j+1,p,ntheta,nzeta)) {
            C = (Q11[indx(j+1,p,ntheta,nzeta)]-Q11[indx(j-1+ntheta,p,ntheta,nzeta)])/(dtheta*dtheta);
            setv(&bmat,indx(j,p,ntheta,nzeta),indx(j-1+ntheta,p,ntheta,nzeta),A-C);};
          /* Right Boundary Periodicity */
          if(is_valid(j-1,p,ntheta,nzeta)&&!is_valid(j+1,p,ntheta,nzeta)) {
            C = (Q11[indx(j+1-ntheta,p,ntheta,nzeta)]-Q11[indx(j-1,p,ntheta,nzeta)])/(dtheta*dtheta);
            setv(&bmat,indx(j,p,ntheta,nzeta),indx(j-1,p,ntheta,nzeta),A-C);};
	    ...
	    C = (Q11[indx(j+1,p,ntheta,nzeta)]-Q11[indx(j-1,p,ntheta,nzeta)])/(dtheta*dtheta);
          ...
          //j,p+1
          if(is_valid(j,p-1,ntheta,nzeta)&&is_valid(j,p+1,ntheta,nzeta)) {
            D = (Q22[indx(j,p+1,ntheta,nzeta)]-Q22[indx(j,p-1,ntheta,nzeta)])/(dzeta*dzeta);
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p+1,ntheta,nzeta),B+D);};
          /* Left Boundary Periodicity */
          if(!is_valid(j,p-1,ntheta,nzeta)&&is_valid(j,p+1,ntheta,nzeta)) {
            D = (Q22[indx(j,p+1,ntheta,nzeta)]-Q22[indx(j,p-1+ntheta,ntheta,nzeta)])/(dzeta*dzeta);
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p+1,ntheta,nzeta),B+D);};
          /* Right Boundary Periodicity */
          if(is_valid(j,p-1,ntheta,nzeta)&&!is_valid(j,p+1,ntheta,nzeta)) {
            D = (Q22[indx(j,p+1-ntheta,ntheta,nzeta)]-Q22[indx(j,p-1,ntheta,nzeta)])/(dzeta*dzeta);
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p+1-ntheta,ntheta,nzeta),B+D);};
	    ...
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p-1,ntheta,nzeta),B-D);};
          ...
          //j,p
          setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p,ntheta,nzeta),-2*(A+B)-R[indx(j,p,ntheta,nzeta)]/H[indx(j,p,ntheta,nzeta)]);
          //Source term
    ...
    while (ctime<t_f){
      for(j=0;j<ntheta;j++){
        for(p=0;p<nzeta;p++){
          ...
          //j-1,p
          if(is_valid(j-1,p,ntheta,nzeta)&&is_valid(j+1,p,ntheta,nzeta)) {
	    C = (Q11[indx(j+1,p,ntheta,nzeta)]-Q11[indx(j-1,p,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dtheta*dtheta);
            setv(&bmat,indx(j,p,ntheta,nzeta),indx(j-1,p,ntheta,nzeta),A-C);};
          /* Left Boundary Periodicity */
          if(!is_valid(j-1,p,ntheta,nzeta)&&is_valid(j+1,p,ntheta,nzeta)) {
            C = (Q11[indx(j+1,p,ntheta,nzeta)]-Q11[indx(j-1+ntheta,p,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dtheta*dtheta);
            setv(&bmat,indx(j,p,ntheta,nzeta),indx(j-1+ntheta,p,ntheta,nzeta),A-C);};
          /* Right Boundary Periodicity */
          if(is_valid(j-1,p,ntheta,nzeta)&&!is_valid(j+1,p,ntheta,nzeta)) {
            C = (Q11[indx(j+1-ntheta,p,ntheta,nzeta)]-Q11[indx(j-1,p,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dtheta*dtheta);
            setv(&bmat,indx(j,p,ntheta,nzeta),indx(j-1,p,ntheta,nzeta),A-C);};
	    ...
	    C = (Q11[indx(j+1,p,ntheta,nzeta)]-Q11[indx(j-1,p,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dtheta*dtheta);
          ...
          //j,p+1
          if(is_valid(j,p-1,ntheta,nzeta)&&is_valid(j,p+1,ntheta,nzeta)) {
            D = (Q22[indx(j,p+1,ntheta,nzeta)]-Q22[indx(j,p-1,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dzeta*dzeta);
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p+1,ntheta,nzeta),B+D);};
          /* Left Boundary Periodicity */
          if(!is_valid(j,p-1,ntheta,nzeta)&&is_valid(j,p+1,ntheta,nzeta)) {
            D = (Q22[indx(j,p+1,ntheta,nzeta)]-Q22[indx(j,p-1+ntheta,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dzeta*dzeta);
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p+1,ntheta,nzeta),B+D);};
          /* Right Boundary Periodicity */
          if(is_valid(j,p-1,ntheta,nzeta)&&!is_valid(j,p+1,ntheta,nzeta)) {
            D = (Q22[indx(j,p+1-ntheta,ntheta,nzeta)]-Q22[indx(j,p-1,ntheta,nzeta)])*H[indx(j,p,ntheta,nzeta)]/(dzeta*dzeta);
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p+1-ntheta,ntheta,nzeta),B+D);};
	    ...
	    setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p-1,ntheta,nzeta),B-D);};
          ...
          //j,p
          setv(&bmat,indx(j,p,ntheta,nzeta),indx(j,p,ntheta,nzeta),1-(2*(A+B)+R[indx(j,p,ntheta,nzeta)])*dt);
